    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (!responseV1.ok) {
                ...
                throw new Error(`Model Discovery Failed: v1beta (${response.status}) & v1 (${responseV1.status})`);
            }
        ...
        throw error; // Re-throw to capture details
    }
}
            ...
            if (initialError.message?.includes("404") || initialError.message?.includes("not found") || initialError.message?.includes("403")) {
                try {
                    ...
                    if (discovered) {
                        ...
                        lastErrorDetails = new Error("Aucun modèle compatible trouvé (Discovery returned null)");
                    }
                } catch (discoveryError: any) {
                    console.error("Échec critique de la découverte:", discoveryError);
                    lastErrorDetails = discoveryError;
                }
            }
        }
